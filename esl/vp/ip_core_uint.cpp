...
void Ip_Core::gaussian_blur(sc_core::sc_time &offset)
{
 //  pl_t pl;
   FILE *fp; 
    ...
    uint32_t sigma_uint ;
    read_rom(VP_ADDR_SIGMA_ROM_L + img_per_octave, sigma_uint);
    sigma_t sigma = (double)sigma_uint / (double)(1 << FIXED_POINT_FRACTIONAL_BITS_SIGMA_T);
    //cout << sigma << endl;
  ...
  //  Image kernel(size, 1, 1);
   ...
   // uint32_t sum_kernel = 0;
    
    uint32_t sum_kernel_1 =0;
    read_rom(VP_ADDR_SIGMA_ROM_L + 12 + img_per_octave, sum_kernel_1);
    uint32_t sigma_2 =0;
    read_rom(VP_ADDR_SIGMA_ROM_L + 6 + img_per_octave, sigma_2);
          
        fprintf(fp, "\t%d\n", (int)sigma_2); 
        for (sc_int<16> k = -size/2; k <= size/2; k++) {
          uint16_t val1, val2 = 0;
          val1 = (uint16_t)((double)(std::exp(-(double)(k*k)* (double)(sigma_2) / double(1 << FIXED_POINT_FRACTIONAL_BITS_SIGMA_T)) * (double)(1 << FIXED_POINT_FRACTIONAL_BITS_DATA_T) ));
          uint16_t val1_p = (uint16_t)((((double)val1 * (double)sum_kernel_1)) / double(1 << FIXED_POINT_FRACTIONAL_BITS_SIGMA_T) );
    ...
    for (sc_int<16> x = 0; x < img_width; x+=4) {
       ...
       //cout << img.height - offset_down << y << endl;
          uint32_t sum1 = 0; uint32_t sum2 = 0;
          uint32_t sum3 = 0; uint32_t sum4 = 0;
            for ( sc_int<16> k = 0; k < size; k++) {
                 ...
                 uint16_t pix1, pix2, pix3, pix4,  val1, val2;
                 ...
                 fprintf(fp, "%d\t%d\t", (int)pix1, (int)pix2);
                 fprintf(fp, "%d\t%d\n", (int)pix3, (int)pix4);
                 sum1 += (uint32_t)(((double)pix1 * (double)val1) / double(1 << FIXED_POINT_FRACTIONAL_BITS_DATA_T));
                 sum2 += (uint32_t)(((double)pix2 * (double)val1) / double(1 << FIXED_POINT_FRACTIONAL_BITS_DATA_T));
                 sum3 += (uint32_t)(((double)pix3 * (double)val1) / double(1 << FIXED_POINT_FRACTIONAL_BITS_DATA_T));
                 sum4 += (uint32_t)(((double)pix4 * (double)val1) / double(1 << FIXED_POINT_FRACTIONAL_BITS_DATA_T));
                 //cout << sum << endl;
                 offset += sc_time(DELAY, SC_NS);
            }
          //  tmp.set_pixel(x, y-offset_up, 0, sum);
          write_mem(VP_ADDR_TMP_BRAM_L + 2 *((y-img_offset_up)*img_width + x), (uint16_t)sum1, (uint16_t)sum2);
          write_mem(VP_ADDR_TMP_BRAM_L + 2 *((y-img_offset_up)*img_width + x + 2), (uint16_t)sum3, (uint16_t)sum4);
        }
    }
    fclose(fp);
    ...
    // convolve horizontal
       for (sc_int<16> x = 0; x < img_width; x+=4) {
           ...
           write_mem(VP_ADDR_MAIN_BRAM_L + 2 * (y*img_width + x), (uint16_t)sum1, (uint16_t)sum2);
           write_mem(VP_ADDR_MAIN_BRAM_L + 2 * (y*img_width + x + 2), (uint16_t)sum3, (uint16_t)sum4);
        }
    }
    fclose(fp);
   offset += sc_time(DELAY, SC_NS);
...
void Ip_Core::write_mem(sc_dt::sc_uint<64> addr, uint16_t pix1, uint16_t pix2)
{
	pl_t pl;
	sc_dt::uint64 taddr = addr & 0x00FFFFFF;
	unsigned char buf[4];
	Int16_t_to_Uchar(buf, pix1, pix2);
...
void Ip_Core::read_mem(sc_dt::sc_uint<64> addr, uint16_t& pix1, uint16_t& pix2)
{
	pl_t pl;
	sc_dt::uint64 taddr = addr & 0x00FFFFFF;
	unsigned char buf[4];
	...
	Uchar_to_Int16_t(buf, pix1, pix2);
}

void Ip_Core::read_rom(sc_dt::sc_uint<64> addr, uint32_t& data)    
{
    pl_t pl;
    sc_dt::uint64 taddr = addr & 0x00FFFFFF;
    unsigned char buf[4];
