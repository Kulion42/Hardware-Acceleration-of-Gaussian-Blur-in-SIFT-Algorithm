...
Soft::Soft(sc_core::sc_module_name name, char** strings, int arg_count) : sc_module(name)
{
	SC_THREAD(soft);
...
void Soft::soft()
{
	...
	Image resized_grayscale_image = grayscale_img.resize(grayscale_img.width*2, grayscale_img.height*2, Interpolation::BILINEAR);
	
	//ovde je bila funkcija:
	//ScaleSpacePyramid gaussian_pyramid = generate_gaussian_pyramid(resized_img, sigma_min, num_octaves, scales_per_octave);
    
    write_hard(ADDR_RESET, 1);
    while(!read_hard(ADDR_READY));
    write_hard(ADDR_RESET, 0);
    
    write_hard(ADDR_IMG_WIDTH, resized_grayscale_image.width); 
	write_hard(ADDR_IMG_HEIGHT, resized_grayscale_image.height);
	//write_hard(ADDR_CHANNELS, resized_grayscale_image.channels);
	write_hard(ADDR_SIGMA_MIN, SIGMA_MIN); //const
	write_hard(ADDR_NUM_OCTAVES, N_OCT); //const
	write_hard(ADDR_SCALES_PER_OCTAVE, N_SPO) //const
	
	cout << "IP core registers initialized" << endl;
	
	//------------------------------------------------
	//OVDE JE POTREBNA INICIJALIZACIJA DMA, ako to koristimo
	//------------------------------------------------
	...
	//-----------------------------------------------
	//OVDE JE POTREBNO NEKO CITANJE PODATAK OBRADJENIH OD STRANE IP
	//-------------------------------------------------
...
vector<Keypoint> Soft::find_keypoints_and_descriptors(const Image& img, k_t sigma_min,
...
ScaleSpacePyramid Soft::generate_dog_pyramid(const ScaleSpacePyramid& img_pyramid)
{
...
vector<Keypoint> Soft::find_keypoints(const ScaleSpacePyramid& dog_pyramid, float contrast_thresh,
                                     float edge_thresh)
{
...
ScaleSpacePyramid Soft::generate_gradient_pyramid(const ScaleSpacePyramid& pyramid)
{
...
vector<float> Soft::find_keypoint_orientations(Keypoint& kp, 
...
void Soft::compute_keypoint_descriptor(Keypoint& kp, float theta,
                                 const ScaleSpacePyramid& grad_pyramid,
                                 float lambda_desc)
{
    float pix_dist = MIN_PIX_DIST * std::pow(2, kp.octave);
...
Image Soft::draw_keypoints(const Image& img, const std::vector<Keypoint>& kps)
{
    Image res(img);
    if (img.channels == 1) {
