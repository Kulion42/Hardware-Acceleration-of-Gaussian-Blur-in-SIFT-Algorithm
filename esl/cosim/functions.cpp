...
{
    //uint16_t in1 = (uint16_t)((double)input1 * (double)(1 << FIXED_POINT_FRACTIONAL_BITS_DATA_T));
    //uint16_t in2 = (uint16_t)((double)input2 * (double)(1 << FIXED_POINT_FRACTIONAL_BITS_DATA_T));
    //std::cout << in1 << " "<< in2 << endl;
    buf[0] = (unsigned char) (in >> 24);
    buf[1] = (unsigned char) (in)>> 16;
    buf[2] = (unsigned char) (in >> 8);
    buf[3] = (unsigned char) (in);
}
...
void Uchar_to_Fixed(unsigned char *buf, data_t& output1, data_t& output2)
{
    ...
    out1 += ((uint16_t)buf[0]) << 8;
    out1 += ((uint16_t)buf[1]);
    
    out2 += ((uint16_t)buf[2]) << 8;
    out2 += ((uint16_t)buf[3]);
